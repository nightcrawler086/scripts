...
BEGIN {
	...
	function Get-SingleModeIfgrps {
		Get-NcNetPortIfgrp | Where-Object {$_.Mode -eq "singlemode"}
	}
...
PROCESS {
	ForEach ($Cluster in $Clusters) {
		...
		# our custom object
		$DATAVAR = Get-SingleModeIfgrps

		ForEach ($IFGRP in $DATAVAR) {
	...
	# That object into the array we defined in the BEGIN block
			$OUTPUT += New-Object -TypeName PSObject -Property @{
							Node = $IFGRP.Node;
							Custer = $IFGRP.Cluster;
							IfgrpName = $IFGRP.Ifgrp;
							Mode = $IFGRP.Mode;
							MacAddress = $IFGRP.MacAddress;
							DistributionFunction = $IFGRP.DistributionFunction;
							PortParticipation = $IFGRP.PortParticipation
			}
		}
	}
}
