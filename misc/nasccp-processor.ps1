	...
	ForEach ($R in $TRFILE) {
		...
		If ($T -and $S) {
			...
			ForEach ($P in $SAPROPS) {
				...
				If ( $($T.$P) -ne ""  -and $($T.$P) -ne "0" -and $($T.$P) -notlike "N?A" -and $($T.$P) -notlike "*NAS TR & MIGRATIONS*") {
					$OBJ | Add-Member -NotePropertyMembers @{"${P}Modified"="OriginalSourceFound"}
				} 
				ElseIf ( $($S.$P) -ne "" -and $($S.$P) -ne "0" -and $($S.$P) -notlike "N*A" -and $($T.$P) -notlike "*NAS TR & MIGRATIONS*") {
					...
					$OBJ | Add-Member -NotePropertyMembers @{"${P}Modified"="SourcePropInvalid"}
				}		
			}
			$OUTPUT += $OBJ
			#$STCOUNT++
		...
		ElseIf (!$S -and $T) {
			...
			If ( !$($T.$P) -and $($T.$P) -ne "0" -and $($T.$P) -notlike "N?A" -and $($T.$P) -notlike "*NAS TR & MIGRATIONS*") {
					$OBJ | Add-Member -NotePropertyMembers @{"${P}Modified"="OriginalSourceNotFound"}
			} Else {
		...
		ElseIf ($S -and !$T) {
			...
			ForEach ($P in $SAPROPS) {
				If ( $($S.$P) -ne "" -and $($S.$P) -ne "0" -and $($S.$P) -notlike "N*A" -and $($T.$P) -notlike "*NAS TR & MIGRATIONS*") {
					...
					$OBJ.$P = "$($S.$P)"
					# Add new property showing where the above value is sourced
					$OBJ | Add-Member -NotePropertyMembers @{"${P}Modified"="SourcePropNoTarget"}
				} Else {
					$OBJ.$P = "$($R.'SA SNOW Queue Name')"
					$OBJ | Add-Member -NotePropertyMembers @{"${P}Modified"="TrackerNoTarget"}
				}		
			}
			$OUTPUT += $OBJ
		}
		Else { $OUTPUT += New-Object -TypeName PSObject -Property @{ AlertSuppress = "0";
				AppID = "";
				Apps_Support_Group_Distribution_List = "";
				App_SNOW_Queue = $($R.'SA SNOW Queue Name');
				App_SNOW_QueueModified = "NewTracker";
				Archived_By = "";
				Archived_Date = "";
				Business_Group_Distribution_List = "";
				Business_Sector = "";
				...
				CCP_Record_Modified_By = "";
				CCP_Record_Modify_Date = "";
				Dispatch_Alert_SNOW_Queue = $($R.'SA SNOW Queue Name');
				Dispatch_Alert_SNOW_QueueModified = "NewTracker";
				ID = "";
				NAS_Alias_vFiler = $($R.'3 DNS cname entry');
				NAS_Frame_DR = $($R.'Target Cob VNX Frame');
				NAS_Frame_PROD = $($R.'Target Prod VNX Frame');
				...
				NotificationSetting = "";
				Region = "NAM";
				SA_SNOW_Queue = $($R.'SA SNOW Queue Name');
				SA_SNOW_QueueModified = "NewTracker";
				SA_Support_Group_Distribution_List = $($R.'SA SNOW Queue Name');
				SNOW_ChangeManagement = "";
				SNOW_GreenZoneEnd = "";
				SNOW_GreenZoneFrequency = "";
